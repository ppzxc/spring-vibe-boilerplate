...
### 4. MDC RequestId 필터 제약

필터 배치 모듈: `boilerplate-adapter-input-api`. MDC 키: `requestId`. Spring Security 이전에 실행.

ScopedValue 예외: Logback/SLF4J가 ScopedValue 미지원이므로 MDC(ThreadLocal 기반)를 사용한다. Checkstyle `ThreadLocal` 금지는 애플리케이션 코드 대상이며, SLF4J MDC는 프레임워크 레벨이므로 예외.
