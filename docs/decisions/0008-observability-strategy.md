...
### 4. MDC RequestId 필터 구현 가이드

필터 배치 모듈: `template-adapter-input-api`

동작:
1. `X-Request-ID` 요청 헤더가 존재하면 해당 값을 사용
2. 헤더가 없으면 UUID v7을 생성 (시간 정렬 가능)
3. MDC에 `requestId` 키로 주입
4. 응답 헤더에도 `X-Request-ID`를 포함 (클라이언트 추적용)
5. 요청 완료 후 MDC에서 `requestId` 제거

필터 순서: Spring Security 필터 체인 이전 (모든 요청에 적용). `Ordered.HIGHEST_PRECEDENCE + 10` 수준.

구조화 로깅 연동: `logging.structured.format.console: logstash` 설정에서 MDC 필드가 자동으로 JSON 출력에 포함됨. 별도 logback 패턴 수정 불필요.

ScopedValue 고려: Virtual Thread 환경에서 MDC는 ScopedValue가 아닌 기존 ThreadLocal 기반이나, Logback/SLF4J가 ScopedValue 미지원이므로 MDC 사용이 현실적 선택. Checkstyle `ThreadLocal` 금지 규칙은 애플리케이션 코드 대상이며, SLF4J MDC는 프레임워크 레벨이므로 예외.
