...
* 이중 로딩 방지 안전장치 확보
* 기존 `boilerplate-application-autoconfiguration` 패턴과의 일관성 유지 (ADR-0012)
...
Chosen option: "Option 2: 모듈별 AutoConfiguration 자체 등록", because 각 모듈이 자신의 Bean 등록 책임을 `AutoConfiguration.imports` 파일로 명시적으로 선언하여 Boot 모듈과의 암묵적 결합을 제거하고, 이미 UseCase 조립에 이 패턴을 채택한 `boilerplate-application-autoconfiguration`(ADR-0012)과 일관성을 유지한다.
...
- 현재: `io.github.ppzxc.boilerplate.application.autoconfiguration`
- 변경 후: `io.github.ppzxc.boilerplate.autoconfigure.*`
- application 모듈: `io.github.ppzxc.boilerplate.autoconfigure.application`
- adapter 모듈 패턴: `io.github.ppzxc.boilerplate.autoconfigure.adapter.{direction}.{type}`
...
`@SpringBootApplication`의 기본 `@ComponentScan` 범위(`io.github.ppzxc.boilerplate`)에서 `@Component`, `@Service`, `@Repository` 등의 스테레오타입을 스캔하여 자동 등록한다.
