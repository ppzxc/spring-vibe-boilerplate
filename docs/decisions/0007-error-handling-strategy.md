...
1. **ErrorCode 위치**: `boilerplate-domain` 모듈에 순수 Java enum으로 정의 (Spring 의존 없음)
2. **GlobalExceptionHandler 위치**: `boilerplate-adapter-input-api` 모듈에 `@RestControllerAdvice`로 배치
