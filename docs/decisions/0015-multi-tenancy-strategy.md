...
- Row-level → Schema-per-Tenant: Enterprise 고객 계약 조건 또는 수백 개 이하 테넌트 예상 시
- Schema-per-Tenant → Database-per-Tenant: 규제 요구(데이터 주권, HIPAA BAA) 또는 noisy neighbor 문제 발생 시
