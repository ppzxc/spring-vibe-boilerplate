...
│
│ ── [Outbound Adapters] boilerplate-infrastructure-messaging-kafka ───
│    메시징 구현 — Producer, Consumer, DLQ
│    의존성: boilerplate-domain, Spring Kafka
│    금지: jOOQ, Spring Web
│    ※ boilerplate-infrastructure-persistence-jooq 직접 참조 금지 (순환 참조 방지)
│
├── boilerplate-infrastructure-messaging-kafka/
│   ├── build.gradle.kts
│   └── src/main/java/io/github/ppzxc/boilerplate/
│       └── infrastructure/
│           └── messaging/
│               ├── publisher/                    # Event Publisher 구현체
│               │   └── KafkaTenantEventPublisher.java
│               │       # implements TenantEventPublisher (domain port)
│               │       # Partition Key = Aggregate ID (순서 보장)
│               │
│               ├── consumer/                     # Event Consumer
│               │   └── TenantEventConsumer.java  # 멱등성 보장 (eventId 기반)
│               │
│               ├── dlq/                          # Dead Letter Queue
│               │   └── DlqHandler.java           # Retry(Exponential Backoff) 최종 실패 시
│               │                                 # DLQ 토픽 전송 + 모니터링 알람
│               │
│               └── config/
│                   ├── KafkaProducerConfig.java
│                   └── KafkaConsumerConfig.java   # Retry 정책, DLQ 토픽 설정
│
...
* **[CRITICAL] 인프라 간 순환 참조 금지:** `boilerplate-infrastructure-*` 모듈 간의 직접 참조는 엄격히 금지됩니다. (예: `boilerplate-infrastructure-messaging-kafka` -> `boilerplate-infrastructure-persistence-jooq` 참조 불가)

### 3.3 실용적 매핑(Mapping) 허용 정책
...
### 6.3 단순 CRUD 예외 정책
`[Pragmatic Exception]`
* 비즈니스 로직이 없는 백오피스/어드민의 단순 CRUD 기능에 한해, **`boilerplate-domain`을 거치지 않고 `boilerplate-application-api`에서 `boilerplate-infrastructure-persistence-jooq`를 직접 호출하여 jOOQ Record를 조작하는 것을 제한적으로 허용**합니다. (핵심 도메인의 생산성 저하 방지)
...
* **발행 보장 (Outbox Pattern, At-Least-Once):** 비즈니스 상태 변경과 이벤트 기록을 동일한 로컬 DB 트랜잭션으로 묶어 저장한 후, 별도의 릴레이 워커가 메시지 브로커(Kafka)로 전송하여 유실을 막습니다.
* **순서 보장 (Event Ordering):** 동일 Aggregate에서 발생한 이벤트는 순서가 보장되어야 합니다. 반드시 **Kafka Partition Key로 Aggregate ID를 사용**하십시오.
* **멱등성 (Idempotency):** 네트워크 재시도 등으로 인해 컨슈머가 동일한 이벤트를 2번 이상 수신해도 데이터가 꼬이지 않도록 로직을 멱등하게 설계합니다.
* **장애 복구 (Retry & DLQ):** 컨슈머 실패 시 점진적 재시도(Exponential Backoff)를 수행하며, 최종 실패한 메시지는 반드시 **DLQ(Dead Letter Queue)**로 이동시키고 모니터링 알람을 발생시켜야 합니다.
...
* **[20%] Application Test:** Mocking을 활용하여 Repository 조회, 판단 위임, 저장/이벤트 흐름이 정상적인지 검증.
* **[10%] E2E Test:** 실제 통합 환경의 핵심 크리티컬 패스만 검증.
* **회귀 테스트 강제:** 버그 발생 시 반드시 **Domain Test로 버그 상황을 재현**하여 실패(Red)를 확인한 후 코드를 수정(Green)합니다.
...
- [ ] **`[Ops]` 이벤트 순서 & 멱등성:** Kafka 파티션 키가 명시되었으며, 중복 수신 시에도 데이터가 보호되는가?
