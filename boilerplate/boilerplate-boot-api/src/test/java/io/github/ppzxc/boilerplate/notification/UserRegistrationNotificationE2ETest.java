package io.github.ppzxc.boilerplate.notification;
...
/** Identity 회원가입 → Notification BC 알림 적재 E2E (Phase 5 Goal 1). */
@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT)
@Testcontainers
class UserRegistrationNotificationE2ETest {
  ...
  @Test
  void 회원가입_후_notifications_테이블에_1건_적재() {
    var body =
        Map.of(
            "userName", "알림테스트",
            "email", "notify_e2e@test.com",
    ...
    var row =
        jdbcTemplate.queryForMap(
            "SELECT status, channel FROM notifications WHERE recipient_id = ?", userId);
